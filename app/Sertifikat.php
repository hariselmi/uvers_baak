...
class Sertifikat extends Model
{
    ...
    public function getAll($option=null, $search=null) {
        ...
        if ($userRole != 'admin') {
            # code...
            $results = $this->select('*')->where(['dlt'=>'0', 'jenis_aktivitas'=>'3', 'user_id'=>$userId])->orderBy('created_at');
        }else{
            $results = $this->select('*')->where(['dlt'=>'0', 'jenis_aktivitas'=>'3'])->orderBy('created_at');
        }
