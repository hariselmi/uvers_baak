...
class Lomba extends Model
{
    ...
    public function getAll($option=null, $search=null) {
        ...
        if ($userRole != 'admin') {
            # code...
            $results = $this->select('*')->where(['dlt'=>'0', 'jenis_aktivitas'=>'1', 'user_id'=>$userId])->orderBy('created_at');
        }else{
            $results = $this->select('*')->where(['dlt'=>'0', 'jenis_aktivitas'=>'1'])->orderBy('created_at');
        }
